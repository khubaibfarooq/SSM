...
const ClientsIndex = (props: any) => {
  const { users, plans, roles, zones } = props;
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    if (isEdit) {
      put(route("clients.update", currentUser.id), {
        onSuccess: () => { setModal(false); toast.success("Client updated successfully"); },
      });
    } else {
      post(route("clients.store"), {
        onSuccess: () => { setModal(false); toast.success("Client created successfully"); },
      });
    }
  };
  ...
  const handlePlanSubmit = (e: any) => {
    e.preventDefault();
    planForm.post(route("clients.assign-plan", currentUser.id), {
      onSuccess: () => {
        setPlanModal(false);
  ...
  const handleDeleteUser = () => {
    destroy(route("clients.destroy", currentUser.id), {
      onSuccess: () => { setDeleteModal(false); toast.success("Client deleted successfully"); },
    });
  };
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        ...
        <Modal show={modal} onHide={toggle} centered size="lg">
          <Modal.Header className="bg-light p-3" closeButton>
            <h5 className="modal-title">{isEdit ? "Edit Client" : "Add Client"}</h5>
          </Modal.Header>
...
ClientsIndex.layout = (page: any) => <Layout children={page} />;
